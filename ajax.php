...
if(isset($iVote) && is_numeric($iVote) && isset($votedUserId) && is_numeric($votedUserId) ) 
{
    if( $iVote<=5 && $iVote>=1){
        if(can_vote_user($votedUserId, $userId)) {
            AR::newInstance()->insertUserVote($votedUserId, $userId, $iVote);
        }
    }
    // return updated voting
    $aux_vote  = AR::newInstance()->getUserAvgRating($votedUserId);
    $aux_count = AR::newInstance()->getUserNumberOfVotes($votedUserId);
...
if(isset($iVote) && is_numeric($iVote) && isset($itemId) && is_numeric($itemId) ) 
{
    ...
    if (osc_is_this_category(osc_get_preference('ads_rating', 'plugin_name'), osc_item_category_id())) {
        $aux_vote  = AR::newInstance()->getItemAvgRating(osc_item_id());
        $aux_count = AR::newInstance()->getItemNumberOfVotes(osc_item_id());
        $vote['vote']  = $aux_vote['vote'];
        $vote['total'] = $aux_count['total'];
